...
export default {
  ...
  methods: {
    ...
    hapus(id) {
      ...
      this.$axios
        .post(
          `${process.env.API_URL}/api/v1/admin-masjid/${id}?_method=DELETE`,
          form,
          configDelete
        )
        .then(({ data }) => {
    ...
    cariMasjid(text) {
      ...
      this.$axios
        .post(
          `${process.env.API_URL}/api/v1/admin-masjid/nama/${text}`,
          form,
          configCari
        )
        .then(({ data }) => {
    ...
    ubah(id) {
      ...
      this.$axios
        .post(
          `${process.env.API_URL}/api/v1/admin-masjid/${id}?_method=PUT`,
          formEdit,
          configEdit
        )
        .then(({ data }) => {
    ...
    request({ pagination, filter }) {
      ...
      this.$axios
        .post(
          `${process.env.API_URL}/api/v1/admin-masjid?page=${pagination.page}`,
          token,
          configGet
        )
        .then(({ data }) => {
    ...
    onRowClick(evt, row) {
      ...
      this.$axios
        .post(
          `${process.env.API_URL}/api/v1/admin-masjid/${row.id}`,
          token,
          configGetId
        )
        .then(({ data }) => {
