  ...
  <q-page>
      ...
      <q-card class="bg-teal text-white" style="width: 300px">
        ...
        <q-card-actions align="right" class="bg-white text-teal">
          <q-btn
            flat
            label="Iya"
            @click="downloadLaporan(linkDownloadLaporan)"
          />
          ...
          <div class="row">
            <div class="col-xl-12">
              ...
              <q-select
                class="q-ma-sm"
                filled
                v-model="formNotifikasi.included_segments[0]"
                :options="options"
                label="Kirim ke"
              />
            </div>
    ...
    <div class="row flex flex-center">
      ...
      >
        <q-card-section
          align="right"
          class="q-mr-none q-mt-none q-pt-none q-pb-none q-pr-none"
          ><i
            class="fa fa-times"
            style="cursor:pointer"
            @click="hapusNotif(notif.id)"
          ></i
        ></q-card-section>
        Konten :
        {{
          notif.contents
            ? notif.contents.en
              ? notif.contents.en.substring(0, 50) + ".."
              : notif.contents.id.substring(0, 50) + ".."
            : ""
        }}
        <div v-if="notif.url !== null">URL : {{ notif.url }}</div>
        <q-separator />
        Selesai Pada :
        {{
          moment(notif.completed_at)
            .locale("id")
            .format("dddd HH:MM DD/MM/YY")
        }}<br />
...
export default {
  ...
  methods: {
    ...
    async report() {
      ...
      await this.$axios
        .post(
          `https://onesignal.com/api/v1/players/csv_export?app_id=${process.env.ONESIGNAL_APP_ID}`,
          {},
          config
        )
        .then(response => {
